...
class Agent:
    ...
    def run(self, context: List[Any]):
        ...
        """
        # TODO: Initialize step to 0, status to "running", and final_answer to None
        step = 0
        status = "running"
        final_answer = None
        
        # TODO: Create a while loop that continues while status is "running" and step < self.max_steps
        while status == "running" and step < self.max_steps:
        
            # TODO: Increment step by 1
            step += 1
            
            # TODO: Call self._next_step(context) and update context, status, and final_answer with the returned values
            context, status, final_answer = self._next_step(context)
        
        # TODO: After the loop, check if status is still "running" and set it to "max_steps_reached" if so
        if status == "running":
            status = "max_steps_reached"
        
        # TODO: Return the tuple (context, status, final_answer)
