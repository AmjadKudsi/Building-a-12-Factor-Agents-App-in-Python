...
class Agent:
    ...
    def _next_step(self, context: List[Any]):
        ...
        for fc in function_calls:
            ...
            # TODO: Create a match statement on call_name
            match call_name:

                # TODO: Add a case for "sum_numbers":
                #   - Wrap execution in try/except
                #   - Call sum_numbers(**call_arguments) and store in result
                #   - Set output = json.dumps({"result": result})
                #   - In except block, set output = json.dumps({"result": f"Error: {str(e)}"})
                case "sum_numbers":
                    try:
                ...
                # TODO: Implement similar cases for all the remaining math tools:
                #   - multiply_numbers
                #   - subtract_numbers
                #   - divide_numbers
                #   - power,
                ...
                # TODO: Add a default case (case _):
                #   - Set output = json.dumps({"result": f"Error: Tool {call_name} not found"})
                case _:
                    output = json.dumps({
                        "result": f"Error: Tool {call_name} not found"
                    })

            # TODO: Append a function_call_output dictionary to context with:
